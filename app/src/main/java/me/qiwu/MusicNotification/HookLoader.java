...
public class HookLoader implements IXposedHookLoadPackage {
    //按照实际使用情况修改下面几项的值
    /**
     * 当前Xpo模块的包名,方便寻找apk文件sed
     */
    private final String modulePackage = BuildConfig.APPLICATION_ID;
    /**
     * 宿主程序的包名,过滤无意义的包名,防止无意义的apk文件加载
     */
    private static ArrayList<String> hostAppPackage = new ArrayList<>();
    static {
        hostAppPackage.add("com.netease.cloudmusic");
        hostAppPackage.add(BuildConfig.APPLICATION_ID);
    }
    ...
    public void handleLoadPackage(final XC_LoadPackage.LoadPackageParam loadPackageParam) throws Throwable {
        if (hostAppPackage.contains(loadPackageParam.packageName)) {
